...
ORDERS = [
    r'^J[PNZ]?\s+[A-Z_]+\s*$',
    r'^[A-Z]+\s+D(C\s+(\d+\*)?INTEGER(\s*\(\s*\-?\s*\d+\s*\)\s*)?|S\s+(\d+\*)?INTEGER\s*)$',
    r'^(L[AR]?|ST)\s+\d+\s*,\s*([A-Z]+|\d+(\s*\(\s*\d+\s*\))?)\s*$',
    r'^[ASMDC]R?\s+\d+\s*,\s*([A-Z]+|\d+(\s*\(\s*\d+\s*\))?)\s*$']
    ...
    while True:
        ...
        for regex in ORDERS:
            if match(regex, line):
                ...
                break
        if is_label:
            LABELS[line.split()[0]] = (location, CURRENT_LINE)

# SET PROGRAM STATE BITS
def set_state(target):
    global STATE
    if REGISTER[int(target)] == 0:
        STATE = 0b00
    elif REGISTER[int(target)] > 0:
        STATE = 0b01
    elif REGISTER[int(target)] < 0:
        STATE = 0b10
    else:
...
def get_short_adress(label):
    if match(r'^[A-Z_]+$', label):                                  # LABEL
        return (MEMORY_LABELS[label] - MEMORY_START) // WORD_LENGTH
    elif match(r'^[0-9]+$', label):                                 # ADRESS
        return (int(label) - MEMORY_START) // WORD_LENGTH
    elif match(r'^\-?[0-9]+\([0-9]+\)$', label):                    # ADRESS REGISTER
        label = sub(r'\)', '', label).split('(')
        delta = int(label[0])
...
def interpret(line):
    ...
    if match(ORDERS[1], line):  # VARIABLE DECLARATION DIRECTIVES
        if match(r'^[A-Z_]+\s+D[CS]\s+INTEGER', line):  # SINGULAR VARIABLES
            line = sub(r'[\(\)\n]', ' ', line).split()
            label = line[0]
            order = line[1]
            if order == "DC":
                ...
                store_label(label)
                MEMORY.append(None)
        elif match(r'^[A-Z_]+\s+D[CS]\s+[0-9]+\*INTEGER', line):  # ARRAYS
            line = sub(r'[\)\n]', '', sub(r'\s*\(', '', line)).split()
        ...
        if order == 'L':
            REGISTER[int(target)] = MEMORY[source]  # LOAD CELL VALUE
        elif order == 'LA':
            REGISTER[int(target)] = source * WORD_LENGTH + \
                MEMORY_START  # LOAD CELL ADRESS
        elif order == 'LR':
            # COPY FROM REGISTER TO REGISTER
            REGISTER[int(target)] = REGISTER[int(line[2])]
        elif order == 'ST':
            MEMORY[source] = REGISTER[int(target)]  # STORE IN MEMORY
        ...
        if match('.R', order):
            source = REGISTER[int(source)]
        else:
            source = MEMORY[get_short_adress(line[2])]
        # ADD, SUBTRACT, MULTIPLY, DIVIDE
        if match('^A', order):
            REGISTER[int(target)] += source
        elif match('^S', order):
            REGISTER[int(target)] -= source
        elif match('^M', order):
            REGISTER[int(target)] *= source
...
def dump_all():
    ...
    if STATE < 2:
        formated_state = '0' + formated_state
    registers_text = "STATE:\t" + formated_state + "\nREGISTERS:\nINDEX\tVALUE"
    if U2_VISIBLE:
        registers_text += "\t\tTWO'S COMPLEMENT:"
    registers_text += "\n"
    for x in range(len(REGISTER)):
        registers_text += str(x) + "\t" + str(REGISTER[x])
        if U2_VISIBLE:
            registers_text += "\t\t" + int_to_u2(REGISTER[x])
        registers_text += "\n"
    registers.print_output(registers_text)
    # DUMPING MEMORY
    memory_text = "MEMORY:\nADRESS\tVALUE:\tLABEL:"
    if U2_VISIBLE:
        memory_text += "\tTWO'S COMPLEMENT:"
    memory_text += "\n"
    for x in range(len(MEMORY)):
        memory_text += str(x * WORD_LENGTH + MEMORY_START) + "\t" + \
            str(MEMORY[x]) + "\t" + get_label(x * WORD_LENGTH + MEMORY_START)
        if U2_VISIBLE:
            memory_text += "\t" + int_to_u2(MEMORY[x])
        memory_text += "\n"
...
def int_to_u2(integer):
    ...
    if integer >= 0:
        for i in range(8 * WORD_LENGTH - len(binary)):
            binary = '0' + binary
    return binary
...
class Editor():
    def __init__(self, root):
        ...
        # MENU ITEM FILE
        # tearoff = 0 -> can't be separated from window
    ...
    def highlight(self, background, foreground, tag, event=None):
        ...
        self.editor.tag_add(
            tag, str(CURRENT_LINE), str(
                CURRENT_LINE + EDITOR_WIDTH / 100))
        ...
        self.editor.tag_remove(
            tag, str(
                CURRENT_LINE + EDITOR_WIDTH / 100), END)
...
def run_code(event=None):
    ...
    while True:
        ...
        if program.tell() == location:
            break  # EOF CHECKING
        line = sub(r'\#.+', '', line)  # IGNORE COMMENTS
        if match(r'^\s*$', line):
            continue
        try:
            interpret(line)
...
def run_by_line(event=None):
    ...
    if not BY_LINE_MODE:
        BY_LINE_MODE = True
        editor.editor.tag_remove(
            "error_line", "1.0", END)  # CLEAR ERROR HIGHLIGHT
...
if __name__ == "__main__":
    ...
    run_by_line_button = Button(
        buttons,
        text="RUN BY LINE",
        command=run_by_line)
    next_line_button = Button(
        buttons,
        text="NEXT LINE",
        command=next_line,
        state="disabled")
